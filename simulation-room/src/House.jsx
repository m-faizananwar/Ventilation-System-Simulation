...
function Stairs({ position, steps = 10, height = 5, width = 2, length = 5 }) {
    ...
    // Calculate ramp angle and length for physics collider
    ...
    return (
        <group position={position}>
            ...
            <RigidBody type="fixed">
                <mesh position={[0, height / 2, length / 2]} rotation={[angle, 0, 0]} visible={false}>
...
export function House(props) {
    return (
        <group {...props}>
            ...
            {/* Non-Physics Visual Elements (Windows, Doors, Furniture) */}
            <group>
                {/* Ground Floor */}
                <Door position={[2.5, 0, 10]} args={[3, 4]} open={true} />
                <Window position={[-5, 2.5, 10]} args={[4, 3]} />
                <Door position={[15, 0, 10]} args={[8, 4]} open={false} />

                {/* First Floor */}
                <Window position={[-5, 7.5, 10]} args={[3, 3]} />
                <Window position={[5, 7.5, 10]} args={[3, 3]} />

                {/* Furniture */}
                <FurnitureBox position={[5, 10.5, 5]} args={[4, 1, 4]} color={COLORS.grass} /> {/* Planter */}
                <FurnitureBox position={[-5, 10.5, 5]} args={[2, 0.5, 2]} color={COLORS.furnitureDark} /> {/* Seat */}
            </group>

            {/* Stairs (With internal Physics Ramp) */}
            <Stairs position={[-8, 0, -5]} steps={15} height={5} width={3} length={8} />
            <Stairs position={[-8, 5, -5]} steps={15} height={5} width={3} length={8} />
